...
use app\models\Cash;
class BsWithdrawQueryJob extends Component implements JobInterface{
    ...
    public function execute($queue){
        ...
        $log->info('汇付天下佣金提现回调查询订单', [$this->outTradeNo,var_export($bsTransferOrder,true)]);
        if(!$bsTransferOrder) {
        	$log->info('汇付天下佣金提现回调查询不到待支付订单', []);
            return;
        }
        ...
        try {
            ...
            $log->info('汇付天下佣金提现回调进来了逻辑了', [$batch_no,$reqdate,Bs::CODE_SUCCESS,$bsTransferOrder->fail_retry_count]);
            ...
            if(!$bsTransferOrder->save()){
            	$log->info('汇付天下佣金提现回调保存失败', []);
                throw new \Exception(json_encode($bsTransferOrder->getFirstErrors()));
            }
            if($bsTransferOrder->status == 4){ //支付成功
            	$cash = Cash::findOne(['order_no' => $bsTransferOrder->outTradeNo, 'is_delete' => 0,'is_transmitting'=>1]);
