...
class IntegralCashForm extends BaseModel
{
    ...
    public function save()
    {
        ...
        if (!$payment['is_integral_cash']) {
            return $this->returnApiResultData(ApiCode::CODE_FAIL, '系统未开启提现功能！');
        }

        if ($this->price <= 0) {
            return $this->returnApiResultData(ApiCode::CODE_FAIL, '提现金豆必须大于0！');
        }
        $user = User::findOne(\Yii::$app->user->identity->id);
        if ($user->static_integral < $this->price) {
            return $this->returnApiResultData(ApiCode::CODE_FAIL, '提现金额超出可提现金豆！');
        }
        ...
        $exists = IntegralCash::find()->where([
        ...
        $dayMaxMoney = $payment['integral_day_max_money'];
        if ($dayMaxMoney > -1) {
            $today_cash_price = IntegralCash::find()->where(['user_id' => $user->id, 'mall_id' => \Yii::$app->mall->id])->andWhere(['>', 'created_at', strtotime(date('Y-m-d', time()))])->andWhere(['<', 'status', 3])->sum('price');
        ...
        try {
            ...
            $cash = new IntegralCash();
            $cash->mall_id    = \Yii::$app->mall->id;
            $cash->user_id    = $user->id;
            $cash->price      = $this->price;
            $cash->fact_price = $this->price;
            if ($payment['integral_cash_service_fee'] > 0 && $payment['integral_cash_service_fee'] < 100) {
                $cash->fact_price = (100 - $payment['integral_cash_service_fee']) * $this->price / 100;
            }
            $cash->order_no         = $this->getCashOrder();
            $cash->service_fee_rate = $payment['integral_cash_service_fee'];
            ...
            $res = UserIntegralForm::cashSub($user, floatval($this->price), $cash->id);
    ...
    private function getCashOrder()
    {
        ...
        while (true) {
            ...
            $exist = IntegralCash::find()->where(['mall_id' => \Yii::$app->mall->id, 'order_no' => $order_no])->exists();
            if (!$exist) {
                break;
            }
        }
    ...
    public function getSetting()
    {
        ...
        if (!$setting['is_integral_cash']) {
            return $this->returnApiResultData(ApiCode::CODE_FAIL, '系统未开启提现功能！');
        }

        if (!$setting['integral_cash_type']) {
            return $this->returnApiResultData(ApiCode::CODE_FAIL, '系统未设置金豆提现方式！');
        }
        $cash_type = SerializeHelper::decode($setting['integral_cash_type']);
        $cash__type_name = [];
        ...
        //获取最近一次提现的银行卡
        $lastBankCash = IntegralCash::find()->where([
        ...
        return $this->returnApiResultData(
            ...
            [
                'setting' => [
                    ...
                    'cash_service_fee' => $setting['integral_cash_service_fee'],
                    'min_money' => $setting['integral_min_money'],
                    'day_max_money' => $setting['integral_day_max_money']
                ],
                'user_info' => [
                    'static_integral' => $user->static_integral,
                    'dynamic_integral' => $user->dynamic_integral,
...
// 获取当前周的第几天 周日是0 周一到周六是 1 - 6
        ...
        return $this->returnApiResultData(ApiCode::CODE_SUCCESS, '', ['week_cash_price' => $week_cash_price, 'total_cash_price' => $total_cash_price, 'month_cash_price' => $month_cash_price, 'static_integral' => \Yii::$app->user->identity->static_integral, 'dynamic_integral' => \Yii::$app->user->identity->dynamic_integral]);
    }
