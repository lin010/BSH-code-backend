...
class SettingForm extends BaseModel
{
    ...
    public $integral_cash_type;
    public $integral_day_max_money;
    public $integral_min_money;
    public $integral_cash_service_fee;
    public $is_integral_cash;
    public $wechat_key_pem_path;
    public $wechat_cert_pem_path;
    public $recharge;
    public $bswechat_status;
    public $bspay_wechat_merchid;
    public $bspay_wechat_sys_id;
    public $bspay_wechat_out_acct_id;
    public $bspay_wechat_product_id;
    public $bspay_wechat_public_key;
    public $bspay_wechat_private_key;

    public function rules()
    {
        return [
            [['score_status', 'score_price', 'balance_transfer_status', 'balance_charge_status', 'balance_cash_status', 'is_income_cash','pay_password_status','integral_status','is_integral_cash'], 'integer'],
            [['score_rule'], 'string'],
            [['cash_type','recharge','integral_cash_type'], 'safe'],
            [['day_max_money', 'cash_service_fee', 'min_money','integral_day_max_money', 'integral_cash_service_fee', 'integral_min_money'], 'number'],
            [['wechat_status', 'balance_status','bswechat_status'], 'integer'],
            ...
            [['bspay_wechat_merchid', 'bspay_wechat_sys_id','bspay_wechat_out_acct_id','bspay_wechat_product_id'], 'string', 'max' => 32],
            [['bspay_wechat_public_key', 'bspay_wechat_private_key'], 'string'],
    ...
    public function attributeLabels()
    {
        return [
            ...
            'is_integral_cash' => '开启金豆提现',
            'recharge' => '充值金额',
            'integral_status'=>'金豆券开启状态',
            'bswechat_status'=>'汇付天下微信支付开启状态',
        ]; // TODO: Change the autogenerated stub
    }
    ...
    public function save()
    {
        ...
        if ($this->integral_cash_type) {
            $this->integral_cash_type = SerializeHelper::encode($this->integral_cash_type);
        } else {
            $this->integral_cash_type = [];
        }
    ...
    public function getSetting()
    {
        ...
        if (isset($res['integral_cash_type']) && !empty($res["integral_cash_type"]) && is_string($res['integral_cash_type'])) {
            $res['integral_cash_type'] = SerializeHelper::decode($res['integral_cash_type']);
        } else {
            $res['integral_cash_type'] = [];
        }
        if ($res['integral_cash_type'] == 'false' || $res['integral_cash_type'] == 'true') {
            $res['integral_cash_type'] = [];
        }
