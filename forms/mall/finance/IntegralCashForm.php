...
use app\forms\bs\BsIntegralCashTransfer;
 ...
 * @property Mall $mall
 */
class IntegralCashForm extends BaseModel
{
    public $mall;
    ...
    public function save()
    {
        ...
        $cash = IntegralCash::findOne(['mall_id' => \Yii::$app->mall->id, 'is_delete' => 0, 'id' => $this->id]);
        if (!$cash) {
            return [
    ...
    private function remit($cash)
    {
        ...
        if ($cash->type === 'auto') {
            ...
            $balanceModel->add((float)$cash->fact_price, '用户金豆提现', '', 'user_cash', $cash->id);

        }elseif($cash->type == "bank"){
            $mode = 'bs';
            if($mode == 'bs'){
                $res = BsIntegralCashTransfer::transfer($cash,'用户金豆提现');
            }else{
    ...
    private function reject($cash)
    {
        ...
        $res = UserIntegralForm::cashReject($user, $cash);
