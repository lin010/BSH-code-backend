<template>
	<view class="container">
		...
		<com-nav-bar v-if="sign == 'meituan'" title="提交订单" :status-bar="true" background-color="#ffffff"
			:border="false" color="#000000"></com-nav-bar>
		<com-nav-bar v-else @clickLeft="back" left-icon="back" title="提交订单" :status-bar="true" background-color="#ffffff"
				:border="false" color="#000000"></com-nav-bar>
		...
		<view class="tui-box">
			...
			<!-- 商品详情 -->
			<!-- 0.1 循环出商家 -->
			<view class="tui-top tui-goods-info" v-for="(item,index) in list" :key='index' :style="{marginTop: sign != 'meituan' ? '20rpx' : '0'}">
				<!-- 0.2 循环出商品 -->
				<tui-list-cell v-if="sign != 'meituan'" :hover="false" :lineLeft="false" padding="26rpx 20rpx">
					<view class="tui-goods-title">
						<view class="logo" @tap="toShop(item.mch.id)">
							...
							<span class="name">{{item.mch.name?item.mch.name:"兰亭官方商城"}}</span>
				...
				<view v-for="(its,ids) in item.same_goods_list" class="item-goods" :key='ids' :style="{marginBottom: sign != 'meituan' ? '20rpx' : '0'}">
					<!-- 0.3 循环出规格 -->
					<view v-for="(gItem,gIndex) in its.goods_list" :key='gIndex'>
						<tui-list-cell :hover="false" padding="0">
							<view class="tui-goods-item">
								<image v-if="sign != 'meituan'"
								...
								<view class="tui-goods-center">
									...
									<view class="tui-goods-attr" v-if="sign != 'meituan'">{{groupName(gItem.attr_list)}}</view>
								</view>
				...
				<tui-list-cell :hover="false" v-if="sign != 'meituan'">
				...
				<tui-list-cell v-if="sign != 'meituan'" :hover="false" :lineLeft="false" padding="0">
					<view class="tui-remark-box tui-padding tui-flex">
						<view>{{sign == 'addcredit' ? '充值号码' : '订单备注'}}</view>
						<input :disabled="sign == 'addcredit'" type="text" class="tui-remark" placeholder="选填: 请先和商家协商一致"
			...
			<view class="use-points flex flex-y-center flex-x-between" v-if="showType.use_score>0">
				<view>使用{{currencyAlias.integral_alias}} <view class="xieti">拥有{{currencyAlias.integral_alias}}：{{user_score}} <text class="text"
			...
			<view class="use-points flex flex-y-center flex-x-between" v-if="showType.use_red_envelopes>0">
				<view>使用{{currencyAlias.red_envelope_alias}} <view class="xieti">拥有{{currencyAlias.red_envelope_alias}}：{{user_integral}} <text class="text"
			...
			<view class="use-points flex flex-y-center flex-x-between" v-if="showType.use_shopping_voucher>0">
				<view>
					使用{{currencyAlias.silver_beans_alias}}
					<view class="xieti">
						拥有{{currencyAlias.silver_beans_alias}}：{{shopping_voucher.total}} 
			...
			<view class="use-points" style="overflow: hidden;line-height: 60rpx;" v-if="got_shopping_voucher_num>0">
				...
				确认收货完成，即可获得<text style="color: red;margin: 0 3rpx;">{{got_shopping_voucher_num}}</text>{{currencyAlias.silver_beans_alias}}
			</view>
		</view>
			...
			<scroll-view scroll-y="true" style="max-height: 1000rpx;">
				<view class="coupon-box">
					<view class="coupon-title2">
						{{currencyAlias.red_envelope_alias}}
...
<script>
	...
	export default {
		...
		onLoad(options) {	
			...
			if(this.sign == 'addcredit'){
				this.remark = options.mobile || '';
			}
			/* uni.chooseAddress({
				success(res) {
			...
			if(currencyAliasConfig){
				...
				/* uni.setNavigationBarTitle({
				  title: this.currencyAlias.integral_alias+'卡券充值'
				}); */
			}
			...
			btnPay() { //getpreviewOrder
				...
				this.$http.request({
					...
					if (res.code == 0) {
						uni.removeStorageSync('orderData');
						uni.redirectTo({
							url: `/pages/order/pay?token=${res.data.token}&queue_id=${res.data.queue_id}&sign=${this.sign}`
						})
					} else {
						this.$http.toast(res.msg);
					}
