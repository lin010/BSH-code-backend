		...
		<view v-if="list && list.length>0" class="shopping-main">
			<view v-for="item in list" class="shopping-coupon-group">
				...
				<view class="coupon-item-left">
					...
					<view class="item-name">{{currencyAlias.red_envelope_alias}}</view>
				</view>
				<view class="coupon-item-right">
					<view class="item-title">{{item.desc}}</view>
					<view class="item-plan">
						发放计划：送{{item.period}}{{item.period_unit=='month'?'月':'周'}}，每{{item.period_unit=='month'?'个月':'周'}}送{{item.integral_num}}{{currencyAlias.red_envelope_alias}}，
						<!-- 永久无需渲染有效期 -->
						<text v-if="item.type!=1">有效期{{item.effective_days}}天</text>
						已发放：{{item.finish_period}}{{item.period_unit=='month'?'个月':'周'}}   {{alreadyIntegral(item.finish_period,item.integral_num)}}{{currencyAlias.red_envelope_alias}} 
					</view>
				</view>
			</view>
		</view>
