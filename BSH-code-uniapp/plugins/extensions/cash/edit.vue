<template>
	<view class="app">
						...
						<view v-if="item.is_show">
							...
							<view>
								...
								<view class="attach" v-if="dataForm.type == 'bank' && item.key == 'bank'">
									...
									<input type="text" v-model="dataForm.bank_account" placeholder="点我输入本人银行卡卡号" class="input" maxlength="50" />
									<picker :value="value" mode="multiSelector" @change="picker" @columnchange="columnPicker" :range="multiArray">
										<input type="text" v-model="text" placeholder="点我选择开户行所在地" class="input" maxlength="50" />
									</picker>
								</view>
							</view>
						</view>
		...
		<com-modal :button="button2" :show="modal2" @click="handleClick2" @cancel="hide2" title="提示" content="您没有实名认证,无法使用银行卡提现,是否需要跳转设置？"></com-modal>
		...
		withdrawType() {
			let temp = [
				...
				},
				{
					...
					key: 'balance',
					title: this.currencyAlias.balance_alias+'提现',
					is_show: false
				}
			];
			let arr = [];
			...
			this.$http
				...
				.then(res => {
					...
					if (res.code == 0) {
						...
						this.dataForm.type = setting.cash_type[1];
