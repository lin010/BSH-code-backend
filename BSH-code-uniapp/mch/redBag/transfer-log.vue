<template>
	<view class="app">
		<com-nav-bar @clickLeft="back" left-icon="back" :title="`${currencyAlias.red_envelope_alias}转账明细`" :status-bar="true" :background-color="navBg" :border="false" :color="navCol"></com-nav-bar>
		<view class="app-body">
			<view class="content">
						...
						<view class="items" v-if="dataList && dataList.length">
							<view class="item" v-for="(item, i) in dataList" :key="i">
								<view class="status flex">
									<view class="name font-size-11" v-if="item.type === 2">{{item.nickname}} 转给我</view>
									<view class="name font-size-11" v-else>我转给 {{item.nickname_receiver}}</view>
									<view class="desc font-size-9" :style="{background:'#FF7104'}">{{ item.type === 1?'-':''}}{{ item.src_price }}</view>
								</view>
								<view class="handling-fee flex" :style="{color:'#FF7104'}">
									<view class="name">手续费</view>
									<view class="fee" :style="{color:'#FF7104'}">-{{item.handling_fee}}</view>
								</view>
								<view class="arrive flex">
									<view class="name">{{ '实际到账' }}</view>
									<view class="fee" :style="{color:'#FF7104'}">+{{ item.price }}</view>
								</view>
								<view class="arrive " v-if="item.desc">
...
export default {
	...
	methods: {
		getDataList(status, bool) {
			...
			this.$http
				.request({
					url: this.$api.plugin.extensions.integral_transfer.log,
					method: 'POST',
					data: {
						page: current_page,
						limit: pageSize,
