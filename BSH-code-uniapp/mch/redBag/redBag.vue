	...
	<view class="income-root">
		...
		<view style="margin-top: 250rpx;margin-bottom: 50rpx;">
			<view class="detail-box" v-for="(item,index) in list" :key='index'>
				<view class="detail-item-box">
					<view class="price flex flex-x-between">
						...
						<view>剩余{{currencyAlias.red_envelope_alias}}： <text :style="{color: '#FF7104'}">{{item.current_integral}}</text> </view>
					</view>
			...
			<view class="exchange-btn" >
				<view class="btn-child1" @click="toCash">{{currencyAlias.red_envelope_alias}}提现</view>
				<view class="btn-child2" @click="toCashList">提现记录</view>
				<view class="btn-child2" @click="toTransfer">转账</view>
				<view class="btn-child2" @click="toTransferLog">转账记录</view>
			</view>
		</view>
...
<script>
	...
	export default {
		...
		onLoad(options) {
			...
			if(currencyAliasConfig){
				...
				uni.setNavigationBarTitle({
					  title: this.currencyAlias.red_envelope_alias+'明细'
					});
				this.asseText =this.currencyAlias.red_envelope_alias;
				this.assetList = [
					{
		...
		methods: {
			...
			toCashList(){
				uni.navigateTo({
					url:'/mch/redBag/cash-details'
				})
			},
			toTransfer(){
				uni.navigateTo({
					url:'/mch/redBag/transfer'
				})
			},
			toTransferLog(){
				uni.navigateTo({
